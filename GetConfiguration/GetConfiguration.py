...
def get_char(config):
    allconf = ['CONF']
    diskconf = ['DISK']
    smartwords = ['Model', 'Power On Hours', 'Power On Count', 'Firmware', 'Disk Size', 'Temperature']
    i = 1
    ...
    while i <= smart[len(smart)-1][0] and i < 4:
        diskconf.append((("Диск", i+1),("Наименование", smart[ifs][2]), ("Прошивка", smart[ifs+1][2]), ("Размер", smart[ifs+2][2].split(" (")[0]), ("Время работы", smart[ifs+3][2].replace("hours", "Часов")), ("Включён", smart[ifs+4][2].replace("count","Раз")), ("Температура", smart[ifs+5][2]), ("Состояние", status[i][1])))
        i +=1
        ifs +=5
